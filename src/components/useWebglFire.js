// 原版 useWebglFire.js
// 改动：1) slider 固定 1.0 2) 始终 active 3) 跳过 MAX_IDLE 自动停止逻辑

import { onMounted, onBeforeUnmount, nextTick } from 'vue'
import { VERT, FRAG_SIM, FRAG_BLUR, FRAG_COMP } from './fire-shaders.js'

export function useWebglFire(canvasRef) {
  let gl = null
  let canvasEl = null
  let rafId = null
  ...
  function init() {
    ...
    if (!ctx) return
    gl = ctx; canvasEl = canvas
    canvas.addEventListener('webglcontextlost', onContextLost)
    compilePrograms()
    if (!programsReady) return
    resizeObserver = new ResizeObserver(() => { clearTimeout(resizeDebounce); resizeDebounce = setTimeout(resize, 80) })
  ...
  function makeFBO() {
    const fbo = gl.createFramebuffer(); const tex = gl.createTexture()
    gl.bindFramebuffer(gl.FRAMEBUFFER, fbo); gl.bindTexture(gl.TEXTURE_2D, tex)
  ...
  function destroyFBO(entry) { if (!gl || !entry) return; gl.deleteFramebuffer(entry.fbo); gl.deleteTexture(entry.tex) }
  ...
  function render(t) {
    ...
    gl.uniform2f(U.blurDir, 1.0, 0.0); gl.uniform1f(U.blurExt, 1.0)
    gl.bindTexture(gl.TEXTURE_2D, simB.tex); gl.uniform1i(U.blurTex, 0)
    gl.drawArrays(gl.TRIANGLES, 0, 6)

    gl.bindFramebuffer(gl.FRAMEBUFFER, blurV.fbo)
    gl.uniform2f(U.blurDir, 0.0, 1.0); gl.uniform1f(U.blurExt, 0.0)
